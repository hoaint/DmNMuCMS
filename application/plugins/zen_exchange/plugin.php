<?php
    ...
    class _plugin_zen_exchange extends controller implements pluginInterface
    {
        private $pluginaizer;
        private $vars = [];
                ...
                if($this->vars['plugin_config'] != false && !empty($this->vars['plugin_config'])){
                    ...
                    if($this->vars['plugin_config']['active'] == 0){
                        ...
                        if(count($_POST) > 0){
                            ...
                            $zen = isset($_POST['zen']) ? (int)$_POST['zen'] : '';
                            list($z, $c) = explode('/', $this->vars['plugin_config']['ratio']);
                            $this->vars['times'] = floor($zen / $z);
                            ...
                            if(!$this->pluginaizer->{'M' . $this->pluginaizer->get_plugin_class()}->check_connect_stat($this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server'])))
                                ...
                                if($this->check_valid_id($char)){
                                    if($zen > $this->vars['plugin_config']['max_zen']){
                                        echo $this->pluginaizer->jsone(['error' => sprintf(__('Maximum Exchange At Once: %s Zen'), $this->pluginaizer->website->zen_format($this->vars['plugin_config']['max_zen']))]);
                                    } else{
                                        if($zen < $z){
                                            echo $this->pluginaizer->jsone(['error' => sprintf(__('Minimum Exchange: %s Zen'), $this->pluginaizer->website->zen_format($z))]);
                                        } else{
                                            if(isset($this->vars['char_data']['Money'])){
                                                if($this->vars['total'] > $this->vars['char_data']['Money']){
                                                    echo $this->pluginaizer->jsone(['error' => sprintf(__('You have insufficient amount of zen on this character. Need: %s Zen'), $this->pluginaizer->website->zen_format($this->vars['total']))]);
                                                    return;
                                                }
                                            }
                                            if(isset($this->vars['warehouse_data']['Money'])){
                                                if($this->vars['total'] > $this->vars['warehouse_data']['Money']){
                                                    echo $this->pluginaizer->jsone(['error' => sprintf(__('You have insufficient amount of zen in warehouse. Need: %s Zen'), $this->pluginaizer->website->zen_format($this->vars['total']))]);
                                                    return;
                                                }
                                            }
                                            if(isset($this->vars['wallet_data']['Money'])){
                                                if($this->vars['total'] > $this->vars['wallet_data']['Money']){
                                                    echo $this->pluginaizer->jsone(['error' => sprintf(__('You have insufficient amount of zen in zen wallet. Need: %s Zen'), $this->pluginaizer->website->zen_format($this->vars['total']))]);
                                                    return;
                                                }
                                            }
                                            $this->pluginaizer->{'M' . $this->pluginaizer->get_plugin_class()}->update_zen($this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server']), $char, $this->vars['total']);
                                            $this->pluginaizer->Maccount->add_account_log('Reward ' . $this->pluginaizer->website->translate_credits($this->vars['plugin_config']['reward_type'], $this->pluginaizer->session->userdata(['user' => 'server'])) . ' From Zen Exchange', $this->vars['reward'], $this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server']));
                                            $this->pluginaizer->website->add_credits($this->pluginaizer->session->userdata(['user' => 'username']), $this->pluginaizer->session->userdata(['user' => 'server']), $this->vars['reward'], $this->vars['plugin_config']['reward_type'], false, $this->pluginaizer->session->userdata(['user' => 'id']));
        ...
        public function admin()
        {
            ...
            if($this->pluginaizer->session->is_admin()){
                ...
                $this->vars['js'] = $this->config->base_url . 'assets/plugins/js/zen_exchange.js';
                //load template
        ...
        public function install()
        {
            ...
            if($this->pluginaizer->session->is_admin()){
                ...
                $this->pluginaizer->set_about()->add_plugin(['installed' => 1, 'module_url' => $this->config->base_url . str_replace('_', '-', $this->pluginaizer->get_plugin_class()), //link to module
                    ...
                    'description' => 'Here you can exchange zen for web or game currency.' //description which will see user
                ]);
                //create plugin config template
                $this->pluginaizer->create_config(['active' => 0, 'max_zen' => 2000000, 'ratio' => "2000/5", 'reward_type' => 1]);
                //check for errors
