			...
			<div class="box-style1" style="margin-bottom:55px;">
				...
				<div class="entry">
					...
					$(document).ready(function(){
						$('button[id^="buy_payu_"]').on('click', function () {
							payU.checkout($(this).attr('id').split('_')[2]);
						});
					});
					</script>
					<?php
					if(isset($error)){
						...
						if(!empty($packages_payu)){
							foreach($packages_payu as $packages){
								echo '<ul id="paypal-options">
										<li>
											...
											<button class="right custom_button" id="buy_payu_' . $packages['id'] . '" style="margin-top: 8px;" value="buy_paypal_' . $packages['id'] . '">' . __('Buy Now') . '</button>
										</li>
								</ul>';
							}
						} 
						else{
							echo '<div class="i_note">' . __('No PayU Packages Found.') . '</div>';
						}
					}
					?>
				</div>
