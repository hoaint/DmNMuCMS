...
<div id="content" class="span10">
	...
	<div class="row-fluid">
        <div class="span12">
            <ul class="nav nav-pills">
				...
				<li><a href="<?php echo $this->config->base_url;?>payu/logs" role="tab">Logs</a></li> 
			</ul>
	...
	var payU = new payU();
	payU.setUrl('<?php echo $this->config->base_url . $this->request->get_controller();?>');
	$(document).ready(function(){
		$('form[id^="payu_settings_form_"]').on("submit", function (e){
			e.preventDefault();
			payU.saveSettings($(this));
		});
		$("#donate_sortable_payu").find("tbody#donate_sortable_content").sortable({
			placeholder: 'ui-state-highlight',
			opacity: 0.6,
			cursor: 'move',
			update: function() { payU.saveOrder(); }
		});
	});
	</script>
	<?php endif;?>
            ...
            <div class="tab-content">
                ...
                <div role="tabpanel" class="tab-pane fade in <?php if($i == 1){?>active<?php }?>" id="<?php echo $key;?>">
                    ...
                    <div class="box-content">
                        <form class="form-horizontal" method="POST" action="" id="payu_settings_form_<?php echo $key;?>">
                            ...
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    ...
                                    <p class="help-block">Use PayU module.</p>
                                </div>
                            </div>
							<div class="control-group">
								<label class="control-label" for="user_id">User Id </label>
								<div class="controls">
									<input type="text" class="span6 typeahead" id="user_id" name="user_id" value="<?php echo $val['user_id']; ?>"/>
									<p class="help-block">Your user id in payu payment system.</p>
								</div>
							</div>
							<div class="control-group">
								<label class="control-label" for="auth_key">Auth Key </label>
								<div class="controls">
									<input type="text" class="span6 typeahead" id="auth_key" name="auth_key" value="<?php echo $val['auth_key']; ?>"/>
									<p class="help-block">Your integration auth key</p>
								</div>
							</div>
							<div class="control-group">
								<label class="control-label" for="md5_key1">1st MD5 Key </label>
								<div class="controls">
									<input type="text" class="span6 typeahead" id="md5_key1" name="md5_key1" value="<?php echo $val['md5_key1']; ?>"/>
									<p class="help-block">Your integration 1st md5 key</p>
								</div>
							</div>
							<div class="control-group">
								<label class="control-label" for="md5_key2">2nd MD5 Key </label>
								<div class="controls">
									<input type="text" class="span6 typeahead" id="md5_key2" name="md5_key2" value="<?php echo $val['md5_key2']; ?>"/>
									<p class="help-block">Your integration 2nd md5 key</p>
								</div>
							</div>
							<div class="control-group">
                            ...
                            <div class="form-actions">
                                <button type="submit" class="btn btn-primary" name="edit_payu_settings" id="edit_payu_settings">Save changes</button>
                            </div>
                        </form>
                    </div>
                </div>
				...
				<div role="tabpanel" class="tab-pane fade in" id="packages">
                    <div class="box-header well">
                        <h2><i class="icon-edit"></i> PayU Packages</h2>
                    </div>
                    <div class="box-content">
                        <div id="donate_package_list">
                            <table class="table table-striped table-bordered bootstrap-datatable datatable" id="donate_sortable_payu">
                                <thead>
                                <tr>
                                    <th>Title</th>
                                    <th>Price</th>
                                ...
                                <?php foreach ($packages_payu as $pack): ?>
                                    <tr id="<?php echo $pack['id']; ?>">
                                        ...
                                        <td class="center">
											<select id="pack_currency_<?php echo $pack['id']; ?>" class="input-medium">
												<option value="USD" <?php if ('USD' == $pack['currency']){ ?>selected="selected"<?php } ?>>USD</option>
												<option value="EUR" <?php if ('EUR' == $pack['currency']){ ?>selected="selected"<?php } ?>>EUR</option>
												<option value="PLN" <?php if ('PLN' == $pack['currency']){ ?>selected="selected"<?php } ?>>PLN</option>
											</select>
										</td>
                                        ...
                                        <td class="center">
                                            <?php if ($pack['status'] == 1): ?>
                                                <a class="btn btn-danger" href="#" id="status_button_<?php echo $pack['id']; ?>" onclick="payU.changeStatus(<?php echo $pack['id']; ?>, 0);">
                                                    <i class="icon-edit icon-white"></i> Disable
                                                </a>
                                            <?php else: ?>
                                                <a class="btn btn-success" href="#" id="status_button_<?php echo $pack['id']; ?>" onclick="payU.changeStatus(<?php echo $pack['id']; ?>, 1);">
                                                    <i class="icon-edit icon-white"></i> Enable
                                                </a>
                                            <?php endif; ?>
                                            <a class="btn btn-info" href="#" onclick="payU.edit(<?php echo $pack['id']; ?>);">
                                                <i class="icon-edit icon-white"></i> Edit
                                            </a>
                                            <a class="btn btn-danger" href="#" onclick="payU.delete(<?php echo $pack['id']; ?>);">
                        ...
                        <table class="table table-striped table-bordered bootstrap-datatable datatable">
                            ...
                            <td class="center">
								...
								<td>
									<a class="btn btn-success" href="#" onclick="payU.addPackage();">
