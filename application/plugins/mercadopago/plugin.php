    ...
    class _plugin_mercadopago extends controller implements pluginInterface
    {
            ...
            if($this->pluginaizer->session->is_user()){
                ...
                if($this->vars['plugin_config'] != false && !empty($this->vars['plugin_config'])){
                    ...
                    if($this->vars['plugin_config']['active'] == 0){
                        ...
                        $this->load->model('application/plugins/mercadopago/models/mercadopago');
                        $this->vars['packages_mercadopago'] = $this->pluginaizer->Mmercadopago->load_packages(true);
                    }
                } else{
                    $this->vars['config_not_found'] = __('Plugin configuration not found.');
                }
            ...
            if($this->pluginaizer->session->is_user()){
                ...
                if($this->vars['plugin_config'] != false && !empty($this->vars['plugin_config'])){
                    ...
                    if($this->vars['plugin_config']['active'] == 0){
                        ...
                        if($id == '')
                            echo $this->pluginaizer->jsone(['error' => __('Invalid MercadoPago package.')]); else{
                            if($this->vars['package'] = $this->pluginaizer->Mmercadopago->check_package($id)){
                                require_once(APP_PATH . DS . 'plugins' . DS . $this->pluginaizer->get_plugin_class() . DS . 'libraries' . DS . 'mercadopago.php');
                                $this->vars['mp'] = new MP(1, $this->vars['plugin_config']['client_id'], $this->vars['plugin_config']['client_secret']);
                                ...
                                if($this->pluginaizer->Mmercadopago->insert_order(filter_var($fields['payment_amount'], FILTER_SANITIZE_NUMBER_INT), $fields['currency'], $this->vars['package']['reward'], $fields['item_number'], $fields['username'], $fields['server'])){
										$order_data = [
										"items" => [[
										...
										"notification_url" => $this->config->base_url . str_replace('_', '-', $this->pluginaizer->get_plugin_class()) . '/callback/'.$this->pluginaizer->session->userdata(['user' => 'server']),
                ...
                if($merchant_order_info["status"] == 200){
                    ...
                    if($merchant_order_info["response"]['status'] == 'closed'){
                        ...
                        if($transaction_amount_payments >= $merchant_order_info["response"]["total_amount"]){
                            if(!$this->pluginaizer->Mmercadopago->check_order_number($merchant_order_info["response"]['external_reference'])){
                                ...
                                if($this->pluginaizer->Mmercadopago->check_completed_transaction($merchant_order_info["response"]['external_reference'])){
                                    $this->writelog('Order already processed: ' . $merchant_order_info["response"]['external_reference'], 'mercadopago');
                                    $this->writelog('order info: ' . print_r($merchant_order_info, true), 'mercadopago');
                                    header("HTTP/1.1 200 OK");
                                } else{
                                    $this->pluginaizer->Mmercadopago->insert_transaction_status($_GET["id"], $merchant_order_info["response"]['external_reference']);
                                    $this->pluginaizer->Maccount->add_account_log('Reward ' . $this->pluginaizer->website->translate_credits($this->vars['plugin_config']['reward_type'], $this->pluginaizer->Mmercadopago->order_details['server']) . ' MercadoPago', $this->pluginaizer->Mmercadopago->order_details['credits'], $this->pluginaizer->Mmercadopago->order_details['account'], $this->pluginaizer->Mmercadopago->order_details['server']);
                                    $this->pluginaizer->website->add_credits($this->pluginaizer->Mmercadopago->order_details['account'], $this->pluginaizer->Mmercadopago->order_details['server'], $this->pluginaizer->Mmercadopago->order_details['credits'], $this->vars['plugin_config']['reward_type'], false, $this->pluginaizer->Mmercadopago->get_guid($this->pluginaizer->Mmercadopago->order_details['account'], $this->pluginaizer->Mmercadopago->order_details['server']));
                                    header("HTTP/1.1 200 OK");
                                }
                            }
                        } else{
        ...
        public function admin()
        {
            ...
            if($this->pluginaizer->session->is_admin()){
                ...
                $this->vars['plugin_config'] = $this->pluginaizer->plugin_config();
                $this->vars['packages_mercadopago'] = $this->pluginaizer->Mmercadopago->load_packages();
                        ...
                        if($currency == '')
                            ...
                            if($server == '')
                                ...
                                if($reward == '')
                                    ...
                                    if($id = $this->pluginaizer->Mmercadopago->add_package($title, $price, $currency, $reward, $server)){
                        ...
                        if($price == '')
                            ...
                            if($currency == '')
                                ...
                                if($server == '')
                                    ...
                                    if($reward == '')
                                        echo $this->pluginaizer->jsone(['error' => 'Invalid package reward']); else{
                                        if($this->pluginaizer->Mmercadopago->check_package($id)){
                                            $this->pluginaizer->Mmercadopago->edit_package($id, $title, $price, $currency, $reward, $server);
        ...
        public function delete_package()
        {
            ...
            if($this->pluginaizer->session->is_admin()){
                ...
                if($id == '')
                    echo $this->pluginaizer->jsone(['error' => 'Invalid package id']); else{
                    if($this->pluginaizer->Mmercadopago->check_package($id)){
                        $this->pluginaizer->Mmercadopago->delete_package($id);
            ...
            if($this->pluginaizer->session->is_admin()){
                ...
                if($id == '')
                    ...
                    if($status == '')
                        ...
                        if($this->pluginaizer->Mmercadopago->check_package($id)){
                            $this->pluginaizer->Mmercadopago->change_status($id, $status);
        ...
        public function save_order()
        {
            ...
            if($this->pluginaizer->session->is_admin()){
                ...
                $this->load->helper('website');
                $this->load->model('application/plugins/mercadopago/models/mercadopago');
                $this->pluginaizer->Mmercadopago->save_order($_POST['order']);
            } else{
                echo $this->pluginaizer->jsone(['error' => 'Please login first!']);
            }
        }
            ...
            if($this->pluginaizer->session->is_admin()){
                ...
                if(isset($_POST['search_mercadopago_transactions'])){
                    ...
                    if($acc == ''){
                        ...
                        $this->vars['logs'] = $this->pluginaizer->Mmercadopago->load_transactions(1, 25, $acc, $server);
                        $this->pluginaizer->pagination->initialize(1, 25, $this->pluginaizer->Mmercadopago->count_total_transactions($acc, $server), $this->config->base_url . 'mercadopagologs/%s/' . $acc . '/' . $server . '');
                        $this->vars['pagination'] = $this->pluginaizer->pagination->create_links();
                    }
                } else{
                    $this->vars['logs'] = $this->pluginaizer->Mmercadopago->load_transactions($page, 25, $acc, $server);
                    $lk = '';
                    if($acc != '')
                        $lk .= '/' . $acc;
                    $lk .= '/' . $server;
                    $this->pluginaizer->pagination->initialize($page, 25, $this->pluginaizer->Mmercadopago->count_total_transactions($acc, $server), $this->config->base_url . 'mercadopago/logs/%s' . $lk);
                    $this->vars['pagination'] = $this->pluginaizer->pagination->create_links();
                }
